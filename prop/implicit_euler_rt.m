function psi_n = implicit_euler_rt(H,en,psi,nsteps,dt,plots)
%Explicit Euler in Imaginary time
%    Propagates solution according to
%     \psi(t+dt) = \psi(t) - dt H\psi(t)

fprintf('\nRunning Implicit Euler in Real time\n')
...
for k = 1:nsteps
    A = speye(size(H)) + 1i*dt*H;
    psi_n = A\psi_o;% - 1i*dt*H*psi_o;
    X = meshgrid(diag(en));
    psi_o = psi_n;
...
title('Error in Implicit Euler Real Time')
saveas(fe,['plots/',plots.fn{4},'.png'])
...
if plots.savem
    ...
    wObj = VideoWriter([plots.fn{4},'.avi']);
    open(wObj);
    for j = 1:nsteps
